...
#include "Snowstorm/Core/Math/Noise.h"

#include "Snowstorm/Utilities/FileSystem.h"

#include "Snowstorm/ImGui/ImGui.h"
#include "Snowstorm/Debug/Profiler.h"
#include "Snowstorm/Math/Math.h"
...
namespace Snowstorm {
	...
	{
		SS_SCOPE_TIMER("SceneRenderer::Init");
		...
		// Bloom Compute
		{
...

			{
				...
				SS_CORE_VERIFY(m_BloomComputePass->Validate());
		...
		// Directional Shadow pass
		{
			...
			for (uint32_t i = 0; i < m_Specification.NumShadowCascades; i++)
			{
				...
				SS_CORE_VERIFY(m_DirectionalShadowMapPass[i]->Validate());
				...
				SS_CORE_VERIFY(m_DirectionalShadowMapAnimPass[i]->Validate());
		...
		// Non-directional shadow mapping pass
		{
			...
			SS_CORE_VERIFY(m_SpotShadowPass->Validate());
...

		{
			...
			// specific. Snowstorm defines Set 0 as per-draw - so usually materials. Sets 1-3 are scene/renderer owned.
			//
			...
			// 		SS_CORE_VERIFY(shadowMapRenderPass[i]->Validate());
			...
			// PreDepth
			{
				...
				SS_CORE_VERIFY(m_PreDepthPass->Validate());
				m_PreDepthPass->Bake();
				...
				SS_CORE_VERIFY(m_PreDepthAnimPass->Validate());
				...
				SS_CORE_VERIFY(m_PreDepthTransparentPass->Validate());
			...
			// Geometry
			{
				...
				pipelineSpecification.Shader = Renderer::GetShaderLibrary()->Get("SnowstormPBR_Static");
				...
				pipelineSpecification.Shader = Renderer::GetShaderLibrary()->Get("SnowstormPBR_Transparent");
				...
				pipelineSpecification.Shader = Renderer::GetShaderLibrary()->Get("SnowstormPBR_Anim");
				pipelineSpecification.DepthOperator = DepthCompareOperator::Equal;
			...
			// Selected Geometry isolation (for outline pass)
			{
				...
				SS_CORE_VERIFY(m_SelectedGeometryPass->Validate());
				...
				SS_CORE_VERIFY(m_SelectedGeometryAnimPass->Validate());
...

			{
				...
				// If resources (eg. storage buffers/images) resize, passes need to be invalidated
				// so we can re-create proper descriptors to the newly created replacement resources
				SS_CORE_VERIFY(m_GeometryPass->Validate());
				m_GeometryPass->Bake();
			}

			{
				...
				SS_CORE_VERIFY(m_GeometryAnimPass->Validate());
...

			{
				...
				SS_CORE_VERIFY(m_LightCullingPass->Validate());
		...
		// Deinterleaving
		{
			...
			for (int rp = 0; rp < 2; rp++)
			{
				...
				SS_CORE_VERIFY(m_DeinterleavingPass[rp]->Validate());
		...
		// Hierarchical Z buffer
		{
			...
			Ref<Shader> shader = Renderer::GetShaderLibrary()->Get("SSB");
			...
			SS_CORE_VERIFY(m_HierarchicalDepthPass->Validate());
		...
		// SSR Composite
		{
			...
			SS_CORE_VERIFY(m_SSRCompositePass->Validate());
		...
		// Pre-convolution Compute
		{
			...
			SS_CORE_VERIFY(m_PreConvolutionComputePass->Validate());
		...
		if (m_Specification.EnableEdgeOutlineEffect)
		{
			...
			SS_CORE_VERIFY(m_EdgeDetectionPass->Validate());
		...
		// Composite
		{
			...
			SS_CORE_VERIFY(m_CompositePass->Validate());
		...
		// DOF
		{
			...
			SS_CORE_VERIFY(m_DOFPass->Validate());
			m_DOFPass->Bake();
		}
		...
		// Wireframe
		{
			...
			SS_CORE_VERIFY(m_GeometryWireframePass->Validate());
			...
			SS_CORE_VERIFY(m_GeometryWireframeOnTopPass->Validate());
			...
			SS_CORE_VERIFY(m_GeometryWireframeAnimPass->Validate());
			...
			SS_CORE_VERIFY(m_GeometryWireframeOnTopAnimPass->Validate());
		...
		// Jump Flood (outline)
		{
			...
			SS_CORE_VERIFY(m_JumpFloodInitPass->Validate());
			...
			for (uint32_t i = 0; i < 2; i++)
			{
				...
				SS_CORE_VERIFY(m_JumpFloodPass[i]->Validate());
			...
			if (m_Specification.JumpFloodPass)
			{
				...
				SS_CORE_VERIFY(m_JumpFloodCompositePass->Validate());
		...
		// Grid
		{
			...
			SS_CORE_VERIFY(m_GridRenderPass->Validate());
		...
		// Skybox
		{
			...
			SS_CORE_VERIFY(m_SkyboxPass->Validate());
			m_SkyboxPass->Bake();
		}
		...
		// GTAO
		{
			...
			SS_CORE_VERIFY(m_GTAOComputePass->Validate());
		...
		{
			{
				...
				SS_CORE_VERIFY(m_GTAODenoisePass[0]->Validate());
				...
				SS_CORE_VERIFY(m_GTAODenoisePass[1]->Validate());
			...
			// GTAO Composite
			{
				...
				SS_CORE_VERIFY(m_AOCompositePass->Validate());
		...
		// SSR
		{
			...
			SS_CORE_VERIFY(m_SSRPass->Validate());
			m_SSRPass->Bake();
		}
	...
	void SceneRenderer::InitOptions()
	{
		...
		// Special macros are strictly starting with "__SS_"
		Renderer::SetGlobalMacroInShaders("__SS_REFLECTION_OCCLUSION_METHOD", fmt::format("{}", (int)m_Options.ReflectionOcclusionMethod));
		//Renderer::SetGlobalMacroInShaders("__SS_GTAO_COMPUTE_BENT_NORMALS", fmt::format("{}", (int)m_Options.GTAOBentNormals));
	}
	...
	{
		SS_CORE_ASSERT(!m_Active, "Can't change scenes while rendering");
		m_Scene = scene;
	}
	...
	void SceneRenderer::UpdateGTAOData()
	{
		...
		gtaoData.SSBUVFactor = m_SSROptions.SSBUvFactor;
	...
	void SceneRenderer::BeginScene(const SceneRendererCamera& camera)
	{
		...
		SS_CORE_ASSERT(m_Scene);
		SS_CORE_ASSERT(!m_Active);
		m_Active = true;
		...
		if (m_NeedsResize)
		{
			...
			// SSB
			{
				...
				m_SSROptions.SSBUvFactor = hzbUVFactor;
	...
	{
		SS_CORE_VERIFY(staticMesh->Handle);
		Ref<MeshSource> meshSource = staticMesh->GetMeshSource();
		const auto& submeshData = meshSource->GetSubmeshes();
		for (uint32_t submeshIndex : staticMesh->GetSubmeshes())
		{
	...
	void SceneRenderer::ShadowMapPass()
	{
		...
		for (uint32_t i = 0; i < m_Specification.NumShadowCascades; i++)
		{
			...
			{
				SS_CORE_VERIFY(m_MeshTransformMap.find(mk) != m_MeshTransformMap.end());
	...
	void SceneRenderer::SSBCompute()
	{
		...
		Renderer::BeginGPUPerfMarker(m_CommandBuffer, "SSB");
		Renderer::BeginComputePass(m_CommandBuffer, m_HierarchicalDepthPass);

		auto ReduceSSB = [commandBuffer = m_CommandBuffer, hierarchicalDepthPass = m_HierarchicalDepthPass, hierarchicalDepthTexture = m_HierarchicalDepthTexture.Texture, hzbMaterials = m_SSBMaterials]
		...
		Renderer::BeginGPUPerfMarker(m_CommandBuffer, "SSB-FirstPass");

		// Reduce first 4 mips
		glm::ivec2 srcSize = m_PreDepthPass->GetDepthOutput()->GetSize();
		ReduceSSB(0, 0, { 1.0f / glm::vec2{ srcSize } }, { (glm::vec2{ srcSize } - 0.5f) / glm::vec2{ srcSize } }, true);
		...
		{
			Renderer::BeginGPUPerfMarker(m_CommandBuffer, fmt::format("SSB-Pass-({})", startDestMip));
			srcSize = Math::DivideAndRoundUp(m_HierarchicalDepthTexture.Texture->GetSize(), 1u << uint32_t(startDestMip - 1));
			ReduceSSB(startDestMip, startDestMip - 1, { 2.0f / glm::vec2{ srcSize } }, glm::vec2{ 1.0f }, false);
			Renderer::EndGPUPerfMarker(m_CommandBuffer);
		}
	...
	void SceneRenderer::PreIntegration()
	{
		...
		for (uint32_t mip = 1; mip < visibilityTexture->GetMipLevelCount(); mip++)
		{
			...
			preIntegrationComputePushConstants.SSBResFactor = resFactor * m_SSROptions.SSBUvFactor;
	...
	void SceneRenderer::CreateSSBPassMaterials()
	{
		constexpr uint32_t maxMipBatchSize = 4;
		const uint32_t hzbMipCount = m_HierarchicalDepthTexture.Texture->GetMipLevelCount();

		Ref<Shader> hzbShader = Renderer::GetShaderLibrary()->Get("SSB");

		uint32_t materialIndex = 0;
		m_SSBMaterials.resize(Math::DivideAndRoundUp(hzbMipCount, 4u));
		for (uint32_t startDestMip = 0; startDestMip < hzbMipCount; startDestMip += maxMipBatchSize)
		{
			...
			m_SSBMaterials[materialIndex++] = material;
			...
			for (destMip = startDestMip; destMip < endDestMip; ++destMip)
			{
				...
				material->Set("o_SSB", m_HierarchicalDepthTexture.ImageViews[destMip], index);
			}
			...
			{
				material->Set("o_SSB", m_HierarchicalDepthTexture.ImageViews[hzbMipCount - 1], destMip); // could be white texture?
			}
		}
	}
	...
	void SceneRenderer::CreatePreIntegrationPassMaterials()
	{
		...
		for (uint32_t i = 0; i < visibilityTexture->GetMipLevelCount() - 1; i++)
		{
			...
			material->Set("u_SSB", m_HierarchicalDepthTexture.Texture);
		}
	}
