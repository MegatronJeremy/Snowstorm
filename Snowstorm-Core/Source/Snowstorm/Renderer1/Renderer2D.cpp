...
#include "Snowstorm/Renderer/Pipeline.h"
...
namespace Snowstorm {
	...
	void Renderer2D::Init()
	{
...

		{
			...
			SS_CORE_VERIFY(m_QuadPass->Validate());
			m_QuadPass->Bake();
		...
		// Lines
		{
...

			{
				...
				SS_CORE_VERIFY(m_LinePass->Validate());
				m_LinePass->Bake();
			}

			{
				...
				SS_CORE_VERIFY(m_LineOnTopPass->Validate());
		...
		// Text
		{
			...
			SS_CORE_VERIFY(m_TextPass->Validate());
			m_TextPass->Bake();
	...
	void Renderer2D::BeginScene(const glm::mat4& viewProj, const glm::mat4& view, bool depthTest)
	{
		...
		SS_CORE_TRACE_TAG("Renderer", "Renderer2D::BeginScene frame {}", frameIndex);
	...
	void Renderer2D::EndScene()
	{
		...
		SS_CORE_TRACE_TAG("Renderer", "Renderer2D::EndScene frame {}", frameIndex);
		uint32_t dataSize = 0;
