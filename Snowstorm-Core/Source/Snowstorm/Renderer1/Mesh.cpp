...
#include "Snowstorm/Asset/AssimpMeshImporter.h"
...
#if MESH_DEBUG_LOG
#define SS_MESH_LOG(...) SS_CORE_TRACE_TAG("Mesh", __VA_ARGS__)
#define SS_MESH_ERROR(...) SS_CORE_ERROR_TAG("Mesh", __VA_ARGS__)
#else
#define SS_MESH_LOG(...)
#define SS_MESH_ERROR(...)
#endif
	...
	void MeshSource::DumpVertexBuffer()
	{
		...
		SS_MESH_LOG("------------------------------------------------------");
		SS_MESH_LOG("Vertex Buffer Dump");
		SS_MESH_LOG("Mesh: {0}", m_FilePath);
		for (size_t i = 0; i < m_Vertices.size(); i++)
		{
			auto& vertex = m_Vertices[i];
			SS_MESH_LOG("Vertex: {0}", i);
			SS_MESH_LOG("Position: {0}, {1}, {2}", vertex.Position.x, vertex.Position.y, vertex.Position.z);
			SS_MESH_LOG("Normal: {0}, {1}, {2}", vertex.Normal.x, vertex.Normal.y, vertex.Normal.z);
			SS_MESH_LOG("Binormal: {0}, {1}, {2}", vertex.Binormal.x, vertex.Binormal.y, vertex.Binormal.z);
			SS_MESH_LOG("Tangent: {0}, {1}, {2}", vertex.Tangent.x, vertex.Tangent.y, vertex.Tangent.z);
			SS_MESH_LOG("TexCoord: {0}, {1}", vertex.Texcoord.x, vertex.Texcoord.y);
			SS_MESH_LOG("--");
		}
		SS_MESH_LOG("------------------------------------------------------");
	}
	...
	const Animation* MeshSource::GetAnimation(const uint32_t animationIndex, const Skeleton& skeleton, bool isMaskedRootMotion, const glm::vec3& rootTranslationMask, float rootRotationMask) const
	{
		...
		SS_CORE_ASSERT(animationIndex < m_Animations.size(), "Animation index out of range!");
		SS_CORE_ASSERT(m_Animations[animationIndex], "Attempted to access null animation!");
