...
namespace std {
	template<>
	struct hash<Snowstorm::WeakRef<Snowstorm::Shader>>
	{
		size_t operator()(const Snowstorm::WeakRef<Snowstorm::Shader>& shader) const noexcept
		{
...
namespace Snowstorm {
	...
	void Renderer::Init()
	{
		...
		Renderer::SetGlobalMacroInShaders("__SS_REFLECTION_OCCLUSION_METHOD", "0");
		Renderer::SetGlobalMacroInShaders("__SS_AO_METHOD", fmt::format("{}", ShaderDef::GetAOMethod(true)));
		Renderer::SetGlobalMacroInShaders("__SS_GTAO_COMPUTE_BENT_NORMALS", "0");
		...
		Renderer::GetShaderLibrary()->Load("Resources/Shaders/SSB.glsl");
		Renderer::GetShaderLibrary()->Load("Resources/Shaders/SnowstormPBR_Static.glsl");
		Renderer::GetShaderLibrary()->Load("Resources/Shaders/SnowstormPBR_Transparent.glsl");
		Renderer::GetShaderLibrary()->Load("Resources/Shaders/SnowstormPBR_Anim.glsl");
	...
	{
		SS_PROFILE_THREAD("Render Thread");
	...
	{
		SS_CORE_ASSERT(renderPass, "RenderPass cannot be null!");
	...
	{
		SS_CORE_ASSERT(computePass, "ComputePass cannot be null!");
	...
	void Renderer::SetGlobalMacroInShaders(const std::string& name, const std::string& value)
	{
		...
		{
			SS_CORE_WARN("No shaders with {} macro found", name);
			return;
		}

		SS_CORE_ASSERT(s_GlobalShaderInfo.ShaderGlobalMacrosMap.find(name) != s_GlobalShaderInfo.ShaderGlobalMacrosMap.end(), "Macro has not been passed from any shader!");
		for (auto& [hash, shader] : s_GlobalShaderInfo.ShaderGlobalMacrosMap.at(name))
		{
			SS_CORE_ASSERT(shader.IsValid(), "Shader is deleted!");
