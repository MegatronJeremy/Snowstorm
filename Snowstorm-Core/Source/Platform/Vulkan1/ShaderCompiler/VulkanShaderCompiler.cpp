...
namespace Snowstorm {
	...
	namespace Utils {
		...
		static ShaderUniformType SPIRTypeToShaderUniformType(spirv_cross::SPIRType type)
		{
			...
			SS_CORE_ASSERT(false, "Unknown type!");
	...
	bool VulkanShaderCompiler::Reload(bool forceCompile)
	{
		...
		SS_CORE_VERIFY(source.size(), "Failed to load shader!");

		SS_CORE_TRACE_TAG("Renderer", "Compiling shader: {}", m_ShaderSourcePath.string());
	...
	std::map<VkShaderStageFlagBits, std::string> VulkanShaderCompiler::PreProcessHLSL(const std::string& source)
	{
		...
		for (auto& [stage, shaderSource] : shaderSources)
		{
			...
			{
				SS_CORE_ERROR_TAG("Renderer", error);
			}
	...
	bool VulkanShaderCompiler::CompileOrGetVulkanBinary(VkShaderStageFlagBits stage, std::vector<uint32_t>& outputBinary, bool debug, VkShaderStageFlagBits changedStages, bool forceCompile)
	{
		...
		if (outputBinary.empty())
		{
			...
			if (std::string error = Compile(outputBinary, stage, options); error.size())
			{
				...
				if (outputBinary.empty())
				{
					SS_CONSOLE_LOG_ERROR("Failed to compile shader and couldn't find a cached version.");
				}
				else
				{
					SS_CONSOLE_LOG_ERROR("Failed to compile {}:{} so a cached version was loaded instead.", m_ShaderSourcePath.string(), ShaderUtils::ShaderStageToString(stage));
#if 0
			...
			else // Compile success
			{
				...
				if (!f)
					SS_CORE_ERROR("Failed to cache shader binary!");
	...
	bool VulkanShaderCompiler::TryReadCachedReflectionData()
	{
		...
		bool validHeader = memcmp(&header, "SSSR", 4) == 0;
		SS_CORE_VERIFY(validHeader);
		if (!validHeader)
			return false;
	...
	void VulkanShaderCompiler::Reflect(VkShaderStageFlagBits shaderStage, const std::vector<uint32_t>& shaderData)
	{
		...
		SS_CORE_TRACE_TAG("Renderer", "===========================");
		SS_CORE_TRACE_TAG("Renderer", " Vulkan Shader Reflection");
		SS_CORE_TRACE_TAG("Renderer", "===========================");

		spirv_cross::Compiler compiler(shaderData);
		auto resources = compiler.get_shader_resources();

		SS_CORE_TRACE_TAG("Renderer", "Uniform Buffers:");
		for (const auto& resource : resources.uniform_buffers)
		{
			...
			if (activeBuffers.size())
			{
				...
				SS_CORE_TRACE_TAG("Renderer", "  {0} ({1}, {2})", name, descriptorSet, binding);
				SS_CORE_TRACE_TAG("Renderer", "  Member Count: {0}", memberCount);
				SS_CORE_TRACE_TAG("Renderer", "  Size: {0}", size);
				SS_CORE_TRACE_TAG("Renderer", "-------------------");
			}
		}

		SS_CORE_TRACE_TAG("Renderer", "Storage Buffers:");
		...
		SS_CORE_TRACE_TAG("Renderer", "Push Constant Buffers:");
		for (const auto& resource : resources.push_constant_buffers)
		{
			...
			SS_CORE_TRACE_TAG("Renderer", "  Name: {0}", bufferName);
			SS_CORE_TRACE_TAG("Renderer", "  Member Count: {0}", memberCount);
			SS_CORE_TRACE_TAG("Renderer", "  Size: {0}", bufferSize);
		...
		SS_CORE_TRACE_TAG("Renderer", "Sampled Images:");
		...
		SS_CORE_TRACE_TAG("Renderer", "Separate Images:");
		...
		SS_CORE_TRACE_TAG("Renderer", "Separate Samplers:");
		...
		SS_CORE_TRACE_TAG("Renderer", "Storage Images:");
		...
		SS_CORE_TRACE_TAG("Renderer", "Special macros:");
		for (const auto& macro : m_AcknowledgedMacros)
		{
			SS_CORE_TRACE_TAG("Renderer", "  {0}", macro);
		}
