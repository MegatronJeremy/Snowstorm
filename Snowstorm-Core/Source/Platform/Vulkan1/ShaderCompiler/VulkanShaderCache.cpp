...
#include "Snowstorm/Platform/Vulkan/VulkanShaderUtils.h"

#include "yaml-cpp/yaml.h"
#include "Snowstorm/Utilities/SerializationMacros.h"
...
namespace Snowstorm {
		...
		for (auto& [filepath, shader] : shaderCache)
		{
			...
			for (auto& [stage, stageData] : shader)
			{
				...
				for (auto& header : stageData.Headers)
				{
					...
					SS_SERIALIZE_PROPERTY(HeaderPath, header.IncludedFilePath.string(), out);
					SS_SERIALIZE_PROPERTY(IncludeDepth, header.IncludeDepth, out);
					SS_SERIALIZE_PROPERTY(IsRelative, header.IsRelative, out);
					SS_SERIALIZE_PROPERTY(IsGaurded, header.IsGuarded, out);
					SS_SERIALIZE_PROPERTY(HashValue, header.HashValue, out);
	...
	void VulkanShaderCache::Deserialize(std::map<std::string, std::map<VkShaderStageFlagBits, StageData>>& shaderCache)
	{
		...
		if (handles.IsNull())
		{
			SS_CORE_ERROR("[ShaderCache] Shader Registry is invalid.");
			return;
		}
		...
		if (handles.IsMap())
		{
			SS_CORE_ERROR("[ShaderCache] Old Shader Registry format.");
			return;
		}
		...
		{
			std::string path;
			SS_DESERIALIZE_PROPERTY(ShaderPath, path, shader, std::string());
			for (auto stage : shader["Stages"]) //Stages
			{
				std::string stageType;
				uint32_t stageHash;
				SS_DESERIALIZE_PROPERTY(Stage, stageType, stage, std::string());
				SS_DESERIALIZE_PROPERTY(StageHash, stageHash, stage, 0u);
				...
				for (auto header : stage["Headers"])
				{
					...
					bool isGuarded;
					uint32_t hashValue;
					SS_DESERIALIZE_PROPERTY(HeaderPath, headerPath, header, std::string());
					SS_DESERIALIZE_PROPERTY(IncludeDepth, includeDepth, header, 0u);
					SS_DESERIALIZE_PROPERTY(IsRelative, isRelative, header, false);
					SS_DESERIALIZE_PROPERTY(IsGaurded, isGuarded, header, false);
					SS_DESERIALIZE_PROPERTY(HashValue, hashValue, header, 0u);
