...
namespace std {
	template<>
	struct hash<Snowstorm::IncludeData>
	{
		size_t operator()(const Snowstorm::IncludeData& data) const noexcept
		{
...
namespace Snowstorm {
		...
		while (startOfShaderStage != std::string::npos)
		{
			...
			if (tokens[index] == "#")
			{
				...
				if (tokens[index] == "pragma")
				{
					...
					{
						SS_CORE_VERIFY(tokens[++index] == ":", "Stage pragma is invalid");

						// Skipped ':'
						const std::string_view stage(tokens[++index]);
						SS_CORE_VERIFY(stage == "vert" || stage == "frag" || stage == "comp", "Invalid shader type specified");
						...
						if (isGuarded && alreadyIncluded)
							...
							SS_CORE_WARN("\"{}\" Header does not contain a header guard (#pragma once).", fullPath);
				...
				{
					++index;
					if (tokens[index].rfind("__SS_", 0) == 0) // Snowstorm special macros start with "__SS_"
					{
				...
				else if (tokens[index] == "if" || tokens[index] == "define") // Consider "#if defined()" too?
				{
					...
					{
						if (tokens[i].rfind("__SS_", 0) == 0) // Snowstorm special macros start with "__SS_"
						{
	...
	std::map<VkShaderStageFlagBits, std::string> ShaderPreprocessor::PreprocessShader(const std::string& source, std::unordered_set<std::string>& specialMacros)
	{
		...
		SS_CORE_ASSERT(newSource.size(), "Shader is empty!");
		...
		if constexpr (Lang == ShaderUtils::SourceLang::GLSL)
		{
			...
			SS_CORE_VERIFY(tokens.size() >= 3 && tokens[1] == "version", "Invalid #version encountered or #version is NOT encounted first.");
		...
		SS_CORE_VERIFY(stagePositions.size(), "Could not pre-process shader! There are no known stages defined in file.");
