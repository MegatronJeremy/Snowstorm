...
namespace Snowstorm {
	...
	namespace Utils {
		static VkPrimitiveTopology GetVulkanTopology(PrimitiveTopology topology)
		{
			...
			SS_CORE_ASSERT(false, "Unknown toplogy");
		...
		static VkCompareOp GetVulkanCompareOperator(const DepthCompareOperator compareOp)
		{
			...
			SS_CORE_ASSERT(false, "Unknown Operator");
	...
	{
		SS_CORE_ASSERT(spec.Shader);
		SS_CORE_ASSERT(spec.TargetFramebuffer);
		Invalidate();
	...
	void VulkanPipeline::Invalidate()
	{
		...
		{
			// SS_CORE_WARN("[VulkanPipeline] Creating pipeline {0}", instance->m_Specification.DebugName);

			VkDevice device = VulkanContext::GetCurrentDevice()->GetVulkanDevice();
			SS_CORE_ASSERT(instance->m_Specification.Shader);
			...
			// Vulkan uses the concept of rendering pipelines to encapsulate fixed states, replacing Vulkan's complex state machine
