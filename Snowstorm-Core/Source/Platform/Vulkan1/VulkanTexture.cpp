...
#include "Snowstorm/Asset/TextureImporter.h"
...
namespace Snowstorm {
	...
	namespace Utils {
		...
		static VkSamplerAddressMode VulkanSamplerWrap(TextureWrap wrap)
		{
			...
			SS_CORE_ASSERT(false, "Unknown wrap mode");
		...
		static VkFilter VulkanSamplerFilter(TextureFilter filter)
		{
			...
			SS_CORE_ASSERT(false, "Unknown filter");
			return (VkFilter)0;
		}
	...
	{ 
		...
		SS_CORE_ASSERT(m_Specification.Format != ImageFormat::None);
	...
	void VulkanTexture2D::Invalidate()
	{
		...
		if (m_ImageData)
		{
			...
			SS_CORE_ASSERT(m_ImageData.Data);
	...
	void VulkanTextureCube::Release()
	{
		...
		{
			SS_CORE_TRACE_TAG("Renderer", "Destroying VulkanTextureCube");
	...
	{
		// SS_CORE_VERIFY(buffer.Size == m_GPUAllocationSize);
