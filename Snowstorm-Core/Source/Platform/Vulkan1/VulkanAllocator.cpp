...
#include "Snowstorm/Utilities/StringUtils.h"

#if SS_LOG_RENDERER_ALLOCATIONS
#define SS_ALLOCATOR_LOG(...) SS_CORE_TRACE(__VA_ARGS__)
#else
#define SS_ALLOCATOR_LOG(...)
#endif

#define SS_GPU_TRACK_MEMORY_ALLOCATION 1
...
namespace Snowstorm {
	...
	{
		SS_CORE_ASSERT(m_Device);

		// TODO: Tracking
		SS_CORE_TRACE("VulkanAllocator ({0}): allocating {1}", m_Tag, Utils::BytesToString(requirements.size));

		{
			static uint64_t totalAllocatedBytes = 0;
			totalAllocatedBytes += requirements.size;
			SS_CORE_TRACE("VulkanAllocator ({0}): total allocated since start is {1}", m_Tag, Utils::BytesToString(totalAllocatedBytes));
		}
	...
	{
		SS_CORE_VERIFY(bufferCreateInfo.size > 0);
		...
		{
			SS_CORE_ERROR_TAG("Renderer", "Failed to allocate GPU buffer!");
			SS_CORE_ERROR_TAG("Renderer", "  Requested size: {}", Utils::BytesToString(bufferCreateInfo.size));
			auto stats = GetStats();
			SS_CORE_ERROR_TAG("Renderer", "  GPU mem usage: {}/{}", Utils::BytesToString(stats.Used), Utils::BytesToString(stats.TotalAvailable));
		}

		// TODO: Tracking
		VmaAllocationInfo allocInfo{};
		vmaGetAllocationInfo(s_Data->Allocator, allocation, &allocInfo);
		SS_ALLOCATOR_LOG("VulkanAllocator ({0}): allocating buffer; size = {1}", m_Tag, Utils::BytesToString(allocInfo.size));

		{
			s_Data->TotalAllocatedBytes += allocInfo.size;
			SS_ALLOCATOR_LOG("VulkanAllocator ({0}): total allocated since start is {1}", m_Tag, Utils::BytesToString(s_Data->TotalAllocatedBytes));
		}

#if SS_GPU_TRACK_MEMORY_ALLOCATION
	...
	VmaAllocation VulkanAllocator::AllocateImage(VkImageCreateInfo imageCreateInfo, VmaMemoryUsage usage, VkImage& outImage, VkDeviceSize* allocatedSize)
	{
		...
		if (allocation == nullptr)
		{
			SS_CORE_ERROR_TAG("Renderer", "Failed to allocate GPU image!");
			SS_CORE_ERROR_TAG("Renderer", "  Requested size: {}x{}x{}", imageCreateInfo.extent.width, imageCreateInfo.extent.height, imageCreateInfo.extent.depth);
			SS_CORE_ERROR_TAG("Renderer", "  Mips: {}", imageCreateInfo.mipLevels);
			SS_CORE_ERROR_TAG("Renderer", "  Layers: {}", imageCreateInfo.arrayLayers);
			auto stats = GetStats();
			SS_CORE_ERROR_TAG("Renderer", "  GPU mem usage: {}/{}", Utils::BytesToString(stats.Used), Utils::BytesToString(stats.TotalAvailable));
		}
		...
		SS_ALLOCATOR_LOG("VulkanAllocator ({0}): allocating image; size = {1}", m_Tag, Utils::BytesToString(allocInfo.size));
	...
	void VulkanAllocator::Free(VmaAllocation allocation)
	{
		...
		{
			SS_CORE_ERROR("Could not find GPU memory allocation: {}", (void*)allocation);
		}
#endif
	}
	...
	{
		SS_CORE_ASSERT(image);
		SS_CORE_ASSERT(allocation);
	...
	void VulkanAllocator::DumpStats()
	{
		...
		{
			SS_CORE_WARN("VmaBudget.allocationBytes = {0}", Utils::BytesToString(b.allocationBytes));
			SS_CORE_WARN("VmaBudget.blockBytes = {0}", Utils::BytesToString(b.blockBytes));
			SS_CORE_WARN("VmaBudget.usage = {0}", Utils::BytesToString(b.usage));
			SS_CORE_WARN("VmaBudget.budget = {0}", Utils::BytesToString(b.budget));
		}
		SS_CORE_WARN("-----------------------------------");
	}
