...
namespace Snowstorm::Utils {
	...
	void RetrieveDiagnosticCheckpoints()
	{
...

		{
			...
			vkGetQueueCheckpointDataNV(::Snowstorm::VulkanContext::GetCurrentDevice()->GetGraphicsQueue(), &retrievedCount, data);
			SS_CORE_ERROR("RetrieveDiagnosticCheckpoints (Graphics Queue):");
			for (uint32_t i = 0; i < retrievedCount; i++)
			{
				VulkanCheckpointData* checkpoint = (VulkanCheckpointData*)data[i].pCheckpointMarker;
				SS_CORE_ERROR("Checkpoint: {0} (stage: {1})", checkpoint->Data, StageToString(data[i].stage));
			}
		}
		{
			const uint32_t checkpointCount = 4;
			...
			uint32_t retrievedCount = checkpointCount;
			vkGetQueueCheckpointDataNV(::Snowstorm::VulkanContext::GetCurrentDevice()->GetComputeQueue(), &retrievedCount, data);
			SS_CORE_ERROR("RetrieveDiagnosticCheckpoints (Compute Queue):");
			for (uint32_t i = 0; i < retrievedCount; i++)
			{
				VulkanCheckpointData* checkpoint = (VulkanCheckpointData*)data[i].pCheckpointMarker;
				SS_CORE_ERROR("Checkpoint: {0} (stage: {1})", checkpoint->Data, StageToString(data[i].stage));
