...
#define SS_HAS_AFTERMATH !SS_DIST
...
namespace Snowstorm {
	...
	VulkanPhysicalDevice::VulkanPhysicalDevice()
	{
		...
		{
			SS_CORE_TRACE_TAG("Renderer", "Could not find discrete GPU.");
		...
		if (extCount > 0)
		{
			...
			{
				SS_CORE_TRACE_TAG("Renderer", "Selected physical device has {0} extensions", extensions.size());
				for (const auto& ext : extensions)
				{
					m_SupportedExtensions.emplace(ext.extensionName);
					SS_CORE_TRACE_TAG("Renderer", "  {0}", ext.extensionName);
				}
			}
		}
	...
	Ref<VulkanCommandPool> VulkanDevice::GetThreadLocalCommandPool()
	{
		...
		SS_CORE_VERIFY(m_CommandPools.find(threadID) != m_CommandPools.end());
