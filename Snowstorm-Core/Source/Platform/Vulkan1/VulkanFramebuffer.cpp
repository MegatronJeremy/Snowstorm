...
namespace Snowstorm {
		...
		: m_Specification(specification)
	{
		...
		SS_CORE_ASSERT(specification.Attachments.Attachments.size());
	...
	{
		// SS_CORE_TRACE("VulkanFramebuffer::RT_Invalidate ({})", m_Specification.DebugName);
		...
		for (const auto& attachmentSpec : m_Specification.Attachments.Attachments)
		{
			if (Utils::IsDepthFormat(attachmentSpec.Format))
			{
				...
				else if (m_Specification.ExistingImages.find(attachmentIndex) != m_Specification.ExistingImages.end())
				{
					...
					SS_CORE_ASSERT(Utils::IsDepthFormat(existingImage->GetSpecification().Format), "Trying to attach non-depth image as depth attachment");
			...
			{
				//SS_CORE_ASSERT(!m_Specification.ExistingImage, "Not supported for color attachments");
				...
				else if (m_Specification.ExistingImages.find(attachmentIndex) != m_Specification.ExistingImages.end())
				{
					...
					SS_CORE_ASSERT(!Utils::IsDepthFormat(existingImage->GetSpecification().Format), "Trying to attach depth image as color attachment");
		...
		if (m_DepthAttachmentImage)
		{
			...
			{
				SS_CORE_ASSERT(m_Specification.ExistingImageLayers.size() == 1, "Depth attachments do not support deinterleaving");
			...
			SS_CORE_ASSERT(attachments.back());
		}
