...
namespace Snowstorm {
	...
	void VulkanShader::RT_Reload(const bool forceCompile)
	{
		...
		{
			SS_CORE_FATAL("Failed to recompile shader!");
		}
#endif
	}
	...
	void VulkanShader::CreateDescriptors()
	{
		...
		for (uint32_t set = 0; set < m_ReflectionData.ShaderDescriptorSets.size(); set++)
		{
			...
			for (auto& [binding, storageBuffer] : shaderDescriptorSet.StorageBuffers)
			{
				...
				layoutBinding.pImmutableSamplers = nullptr;
				layoutBinding.binding = binding;
				SS_CORE_ASSERT(shaderDescriptorSet.UniformBuffers.find(binding) == shaderDescriptorSet.UniformBuffers.end(), "Binding is already present!");
			...
			for (auto& [binding, imageSampler] : shaderDescriptorSet.ImageSamplers)
			{
				...
				SS_CORE_ASSERT(shaderDescriptorSet.UniformBuffers.find(binding) == shaderDescriptorSet.UniformBuffers.end(), "Binding is already present!");
				SS_CORE_ASSERT(shaderDescriptorSet.StorageBuffers.find(binding) == shaderDescriptorSet.StorageBuffers.end(), "Binding is already present!");
			...
			for (auto& [binding, imageSampler] : shaderDescriptorSet.SeparateTextures)
			{
				...
				SS_CORE_ASSERT(shaderDescriptorSet.UniformBuffers.find(binding) == shaderDescriptorSet.UniformBuffers.end(), "Binding is already present!");
				SS_CORE_ASSERT(shaderDescriptorSet.ImageSamplers.find(binding) == shaderDescriptorSet.ImageSamplers.end(), "Binding is already present!");
				SS_CORE_ASSERT(shaderDescriptorSet.StorageBuffers.find(binding) == shaderDescriptorSet.StorageBuffers.end(), "Binding is already present!");
			...
			for (auto& [binding, imageSampler] : shaderDescriptorSet.SeparateSamplers)
			{
				...
				SS_CORE_ASSERT(shaderDescriptorSet.ImageSamplers.find(binding) == shaderDescriptorSet.ImageSamplers.end(), "Binding is already present!");
				SS_CORE_ASSERT(shaderDescriptorSet.StorageBuffers.find(binding) == shaderDescriptorSet.StorageBuffers.end(), "Binding is already present!");
				SS_CORE_ASSERT(shaderDescriptorSet.SeparateTextures.find(binding) == shaderDescriptorSet.SeparateTextures.end(), "Binding is already present!");
			...
			for (auto& [bindingAndSet, imageSampler] : shaderDescriptorSet.StorageImages)
			{
				...
				SS_CORE_ASSERT(shaderDescriptorSet.ImageSamplers.find(binding) == shaderDescriptorSet.ImageSamplers.end(), "Binding is already present!");
				SS_CORE_ASSERT(shaderDescriptorSet.SeparateTextures.find(binding) == shaderDescriptorSet.SeparateTextures.end(), "Binding is already present!");
				SS_CORE_ASSERT(shaderDescriptorSet.SeparateSamplers.find(binding) == shaderDescriptorSet.SeparateSamplers.end(), "Binding is already present!");
			...
			SS_CORE_INFO_TAG("Renderer", "Creating descriptor set {0} with {1} ubo's, {2} ssbo's, {3} samplers, {4} separate textures, {5} separate samplers and {6} storage images", set,
	...
	{
		SS_CORE_ASSERT(set < m_DescriptorSetLayouts.size());
	...
	VulkanShader::ShaderMaterialDescriptorSet VulkanShader::CreateDescriptorSets(uint32_t set)
	{
		...
		SS_CORE_ASSERT(m_TypeCounts.find(set) != m_TypeCounts.end());
	...
	VulkanShader::ShaderMaterialDescriptorSet VulkanShader::CreateDescriptorSets(uint32_t set, uint32_t numberOfSets)
	{
		...
		SS_CORE_ASSERT(poolSizes.find(set) != poolSizes.end());
	...
	{
		SS_CORE_ASSERT(set < m_ReflectionData.ShaderDescriptorSets.size());
		SS_CORE_ASSERT(m_ReflectionData.ShaderDescriptorSets[set]);
		...
		{
			SS_CORE_WARN_TAG("Renderer", "Shader {0} does not contain requested descriptor set {1}", m_Name, name);
			return nullptr;
		}
