...
namespace Snowstorm {
	...
	public:
		...
		void Set(const std::string& name, const T& value)
		{
			...
			SS_CORE_ASSERT(decl, "Could not find uniform!");
			if (!decl)
				return;
		...
		{
			auto decl = FindUniformDeclaration(name);
			SS_CORE_ASSERT(decl, "Could not find uniform with name 'x'");
			auto& buffer = m_UniformStorageBuffer;
