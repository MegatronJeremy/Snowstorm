...
namespace Snowstorm {
	...
	void DescriptorSetManager::SetInput(std::string_view name, Ref<UniformBufferSet> uniformBufferSet)
	{
		...
		if (decl)
			...
			SS_CORE_WARN_TAG("Renderer", "[RenderPass ({})] Input {} not found", m_Specification.DebugName, name);
	}
	...
	{
		const RenderPassInputDeclaration* decl = GetInputDeclaration(name);
		SS_CORE_VERIFY(index < decl->Count);
		if (decl)
	...
	bool DescriptorSetManager::Validate()
	{
		...
		for (uint32_t set = m_Specification.StartSet; set <= m_Specification.EndSet; set++)
		{
			...
			{
				SS_CORE_ERROR_TAG("Renderer", "[RenderPass ({})] No input resources for Set {}", m_Specification.DebugName, set);
				return false;
			}
			...
			for (auto&& [name, wd] : shaderDescriptor.WriteDescriptorSets)
			{
				...
				{
					SS_CORE_ERROR_TAG("Renderer", "[RenderPass ({})] No input resource for {}.{}", m_Specification.DebugName, set, binding);
					SS_CORE_ERROR_TAG("Renderer", "[RenderPass ({})] Required resource is {} ({})", m_Specification.DebugName, name, wd.descriptorType);
					return false;
				}
				...
				{
					SS_CORE_ERROR_TAG("Renderer", "[RenderPass ({})] Required resource is wrong type! {} but needs {}", m_Specification.DebugName, resource.Type, wd.descriptorType);
					return false;
				}

				if (resource.Type != RenderPassResourceType::Image2D && resource.Input[0] == nullptr)
				{
					SS_CORE_ERROR_TAG("Renderer", "[RenderPass ({})] Resource is null! {} ({}.{})", m_Specification.DebugName, name, set, binding);
					return false;
				}
			}
		}
	...
	void DescriptorSetManager::Bake()
	{
		...
		if (!Validate())
		{
			SS_CORE_ERROR_TAG("Renderer", "[RenderPass] Bake - Validate failed! {}", m_Specification.DebugName);
			return;
		}
		...
		for (const auto& [set, setData] : InputResources)
		{
			...
			for (uint32_t frameIndex = 0; frameIndex < descriptorSetCount; frameIndex++)
			{
				...
				{
					SS_CORE_INFO_TAG("Renderer", "Render pass update {} descriptors in set {}", writeDescriptors.size(), set);
	...
	{
		SS_PROFILE_FUNC();
		SS_SCOPE_PERF("DescriptorSetManager::InvalidateAndUpdate");
		...
		for (const auto& [set, setData] : InvalidatedInputResources)
		{
			...
			uint32_t frameIndex = perFrameInFlight ? currentFrameIndex : 0;
			{
				...
				for (const auto& [binding, input] : setData)
				{
					...
					switch (input.Type)
					{
						...
						case RenderPassResourceType::Image2D:
						{
							Ref<RendererResource> image = input.Input[0].As<RendererResource>();
							writeDescriptor.pImageInfo = (VkDescriptorImageInfo*)image->GetDescriptorInfo();
							SS_CORE_VERIFY(writeDescriptor.pImageInfo->imageView);
							wd.ResourceHandles[0] = writeDescriptor.pImageInfo->imageView;
							break;
						}
					}
				...
				// SS_CORE_INFO_TAG("Renderer", "RenderPass::Prepare ({}) - updating {} descriptors in set {} (frameIndex={})", m_Specification.DebugName, writeDescriptorsToUpdate.size(), set, frameIndex);
				SS_CORE_INFO_TAG("Renderer", "DescriptorSetManager::InvalidateAndUpdate ({}) - updating {} descriptors in set {} (frameIndex={})", m_Specification.DebugName, writeDescriptorsToUpdate.size(), set, frameIndex);
				VkDevice device = VulkanContext::GetCurrentDevice()->GetVulkanDevice();
	...
	{
		SS_CORE_ASSERT(!m_DescriptorSets.empty());
