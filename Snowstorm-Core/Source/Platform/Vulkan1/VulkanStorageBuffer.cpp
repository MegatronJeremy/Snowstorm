...
#define NO_STAGING 1
	...
	{
		SS_PROFILE_FUNC();
		SS_SCOPE_PERF("VulkanStorageBuffer::RT_SetData");

		// Cannot call SetData if GPU only
		SS_CORE_VERIFY(!m_Specification.GPUOnly);
		...
		{
			SS_SCOPE_PERF("VulkanStorageBuffer::RT_SetData - MemoryMap");
		...
		{
			SS_SCOPE_PERF("VulkanStorageBuffer::RT_SetData - CommandBuffer");
