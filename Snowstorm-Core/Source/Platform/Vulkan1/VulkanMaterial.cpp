...
#include "Snowstorm/Platform/Vulkan/VulkanAPI.h"
...
namespace Snowstorm {
		...
		for (const auto& [name, decl] : m_DescriptorSetManager.InputDeclarations)
		{
			switch (decl.Type)
			{
				...
				case RenderPassInputType::ImageSampler2D:
				{
					...
					SS_CORE_WARN_TAG("Renderer", "VulkanMaterial - setting {} to white 2D texture", name);
					break;
				}
				case RenderPassInputType::ImageSampler3D:
				{
					m_DescriptorSetManager.SetInput(name, Renderer::GetBlackCubeTexture());
					SS_CORE_WARN_TAG("Renderer", "VulkanMaterial - setting {} to black cube texture", name);
					break;
				}
			}
		}

		SS_CORE_VERIFY(m_DescriptorSetManager.Validate());
	...
	void VulkanMaterial::Invalidate()
	{
		...
		{
			SS_CORE_WARN_TAG("Renderer", "[Material] - shader {} has no Set 0!", m_Shader->GetName());
		}
	}
	...
	const ShaderUniform* VulkanMaterial::FindUniformDeclaration(const std::string& name)
	{
		...
		SS_CORE_ASSERT(shaderBuffers.size() <= 1, "We currently only support ONE material buffer!");
